...
public class BasketWebApiTest
{
    ...
    public async Task Get_customer_basket_success()
    {
        ...
        _basketRepositoryMock.Setup(x => x.GetBasketAsync(It.IsAny<string>())).Returns(Task.FromResult(fakeCustomerBasket));
        ...
        Assert.Equal(((actionResult.Result as ObjectResult).Value as CustomerBasket).BuyerId, fakeCustomerId);
    }
    ...
    public async Task Post_customer_basket_success()
    {
        ...
        _basketRepositoryMock.Setup(x => x.UpdateBasketAsync(It.IsAny<CustomerBasket>())).Returns(Task.FromResult(fakeCustomerBasket));
        _identityServiceMock.Setup(x => x.GetUserIdentity()).Returns(fakeCustomerId);
        _serviceBusMock.Setup(x => x.Publish(It.IsAny<UserCheckoutAcceptedIntegrationEvent>()));
    ...
    public async Task Doing_Checkout_Without_Basket_Should_Return_Bad_Request()
    {
        ...
        _basketRepositoryMock.Setup(x => x.GetBasketAsync(It.IsAny<string>())).Returns(Task.FromResult((CustomerBasket)null));
        _identityServiceMock.Setup(x => x.GetUserIdentity()).Returns(fakeCustomerId);
