...
public class OrdersController : ControllerBase
{
    ...
    public OrdersController(IMediator mediator, IOrderQueries orderQueries, IIdentityService identityService, ILogger<OrdersController> logger)
    {
    ...
    public async Task<IActionResult> ShipOrderAsync([FromBody] ShipOrderCommand command, [FromHeader(Name = "x-requestid")] string requestId)
    {
        ...
        if (Guid.TryParse(requestId, out Guid guid) && guid != Guid.Empty)
        {
            var requstShipOrder = new IdentifiedCommand<ShipOrderCommand, bool>(command, guid);

            _logger.LogInformation(
                "----- Sending command: {CommandName} - {IdProperty}: {CommandId} ({@Command})",
                requstShipOrder.GetGenericTypeName(),
                nameof(requstShipOrder.Command.OrderNumber),
                requstShipOrder.Command.OrderNumber,
                requstShipOrder);

            commandResult = await _mediator.Send(requstShipOrder);
        }
        if (!commandResult)
        {
            return BadRequest();
