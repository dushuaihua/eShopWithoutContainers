...
namespace eShopWithoutContainers.Services.Catalog.API.Grpc;
public class CatalogService : CatalogBase
{
    ...
    public override async Task<PaginatedItemsResponse> GetItemsByIds(CatalogItemsRequest request, ServerCallContext context)
    {
        if (!string.IsNullOrEmpty(request.Ids))
        {
            ...
            context.Status = !items.Any() ? new Status(StatusCode.NotFound, $"ids value invalid. Must be comma-separated list of numbers") : new Status(StatusCode.OK, string.Empty);
        ...
        var itemsOnPage = await _catalogContext.CatalogItems.OrderBy(c => c.Name).Skip(request.PageSize * request.PageIndex).Take(request.PageSize).ToListAsync();
    ...
    private PaginatedItemsResponse MapToResponse(List<CatalogItem> items, long count, int pageIndex, int pageSize)
    {
        ...
        items.ForEach(i =>
        {
            var brand = i.CatalogBrand == null ? null : new CatalogApi.CatalogBrand { Id = i.CatalogBrand.Id, Name = i.CatalogBrand.Brand };
            var catalogType = i.CatalogType == null ? null : new CatalogApi.CatalogType { Id = i.CatalogType.Id, Type = i.CatalogType.Type };
